...
describe('Group Accessibility', () => {
    it('created the expected accessibility tree', async () => {
        ...
        function testForNamedRoledCheckedNode(
            node: NamedRoledAndCheckedNode
        ): boolean {
            let found = node.role === 'radio' && node.checked;
            if (
                (/AppleWebKit/.test(window.navigator.userAgent) &&
                    !/Chrome/.test(window.navigator.userAgent)) ||
                /Firefox/.test(window.navigator.userAgent)
            ) {
                found = found && node.name === 'Option 2';
            }
            if (/Chrome/.test(window.navigator.userAgent)) {
                // This is a bug in Chrome: https://bugs.chromium.org/p/chromium/issues/detail?id=1158144#c6
                // This should fail in a good way when it's fixed...
